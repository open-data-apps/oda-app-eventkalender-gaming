...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (
      ...
      let cssUrl = configData.brandingCSSFile;
      const isLocal = ["127.0.0.1", "localhost"].includes(window.location.hostname);
      if (isLocal) {
        if (cssUrl.startsWith("./assets/")) {
          cssUrl = "../" + cssUrl.substring(2);
        } else if (cssUrl.startsWith("assets/")) {
          cssUrl = "../" + cssUrl;
        }
      }
      linkElem.href = cssUrl;
      document.head.appendChild(linkElem);
      console.log("Custom Branding CSS wurde angewendet (" + cssUrl + ").");
    } else if (
