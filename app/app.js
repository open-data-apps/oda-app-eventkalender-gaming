...
function app(configdata = {}, enclosingHtmlDivElement) {
  ...
  if (isKeineDatenquelleKonfiguriert(API_URL)) {
    renderOdasFehler(enclosingHtmlDivElement, new Error("Keine Datenquelle konfiguriert."), qkKontext);
    return null;
  }
  const qkTypWarn = validateUrlTypErwartung(API_URL, "ckan-dl");
  if (qkTypWarn) {
    renderOdasFehler(enclosingHtmlDivElement, new Error(qkTypWarn), qkKontext);
    return null;
  }
  ...
  let disposed = false;
  // QG-B3: eigener Controller je Instanz — laufende Abrufe brechen beim
  // Seitenwechsel ab (das disposed-Flag schuetzt nur die Oberflaeche).
  const qgAbort = new AbortController();
  ...
  // QG-B1: Vorgaenger-Instanz desselben Containers zuerst abraeumen. Ohne das
  // blieben bei Same-Page-Re-Render alte Leaflet-Karte, beide Chart-Instanzen
  // und der AudioContext am Leben (dispose() lief nur beim Seitenwechsel).
  const qgVorheriger = questKalenderInstances.get(enclosingHtmlDivElement);
  if (qgVorheriger) {
    try {
      qgVorheriger.dispose();
    } catch (_e) {}
  }
  ...
  async function loadAllData() {
    ...
    try {
      ...
      const qkFehlerBox = document.getElementById(`${rootId}-tab-content`);
      if (qkFehlerBox) {
        renderOdasFehler(qkFehlerBox, err, {
          url: API_URL,
          label: "Event-API",
          typLabel: "Datei-Download",
          erwarteterTyp: "ckan-dl",
  ...
  function loadLeaflet(callback) {
    ...
    // QG-B2: Fehlerpfad und Wiederverwendung. Vorher fehlte onerror — eine nicht
  ...
  function loadChartJS(callback) {
    ...
    // QG-B2: wie loadLeaflet — Fehlerpfad statt stillem Ausbleiben.
  ...
  // QG-B2: Eine nicht ladbare Bibliothek wird sichtbar gemeldet, statt Karte und
